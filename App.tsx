...
// FIX: Updated import casing to match AiChatModal.tsx and resolve project casing conflict
...
const ImageIcon: React.FC<{className?: string}> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
    </svg>
);
...
const App: React.FC<AppProps> = ({ shopId, shopName }) => {
  ...
  const handleUpdateProductUnit = (productId: string, newUnit: ProductUnit) => handleProductUpdate(productId, { unit: newUnit });
  const handleUpdateProductPackaging = (productId: string, newPackaging: ProductPackaging) => handleProductUpdate(productId, { packaging: newPackaging });
             ...
             {currentUser ? (
                 <div className="flex items-center gap-4">
                    <button
                        ...
                        className="p-2 text-gray-600 hover:text-indigo-600 focus:outline-none rounded-full hover:bg-gray-100 transition-colors"
                        title="Помощник ИИ"
                    >
